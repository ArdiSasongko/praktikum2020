---
title: "Kuis IF-C"
author: "123200103_Ardi Sasongko_KUIS_IF-C"
date: "2022-10-05"
output:
...
```
alasanya karena genres berupa string
...
```{r no6}
tiny_df = movielens %>% select(title, year, genres, rating, timestamp)
head(tiny_df, n = 10)
```
...
```{r no7}
new_table = select(movielens, title, genres, year, rating)
x = filter(new_table, genres=="Romance")
x %>% top_n(20, rating)
```
...
```{r No8}
  Tahun = with(movielens, year)
  hist(Tahun)
  movielens$year[which.max(Tahun)]
  
```
Kesimpulan: Tahun dengan jumlah film terbanyak dalam data movielens adalah sekitar tahun 2016
...
```{r No9}
kategori = vector("character", length(tiny_df$rating))
for (i in 1:length(tiny_df$rating)) {
  ...
  if(tiny_df$rating[i]>2 && tiny_df$rating[i]<3){
    kategori[i] = "Mixed"
  }
}
```
...
```{r No10}
  tiny_df = mutate(tiny_df, kategori = kategori)
  tiny_df
```